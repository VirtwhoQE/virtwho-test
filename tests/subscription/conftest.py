...
def register_assertion():
    ...
    :return:
    """
    owner_error_wrong = ['owner.* is different',
                         'Communication with subscription manager failed']
    owner_error_null = 'system is not registered or you are not root'
    owner_error_disable = ["owner not in|"
                           "owner.* not set|"
                           "virt-who can't be started|"
                           "Communication with subscription manager failed"]
    rhsm_hostname_error = ['Server error attempting a GET to /rhsm/status/',
                           'Communication with subscription manager failed']
    rhsm_port_error = ['Connection refused',
    ...
    rhsm_user_pwd_error_decode = ["codec can't decode",
                                  'Communication with subscription manager failed']

    rhsm_encrypted_password_error = ['Communication with subscription manager failed',
