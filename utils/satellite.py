...
def satellite_manifest_upload(ssh, url, admin_username, admin_password):
    ...
    ret, _ = ssh.runcmd(
        f"hammer -u {admin_username} -p {admin_password} "
...
def satellite_arguments_parser():
    ...
    parser.add_argument(
        "--version", required=True, help="Satellite version, such as '6.9', '6.10'"
    )
    parser.add_argument("--repo", required=True, help="One of ['cdn', 'dogfood']")
    parser.add_argument(
        "--rhel-compose",
        required=True,
